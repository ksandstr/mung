...
#define ACT_SET_PGID 0x1
#define ACT_ACCESS_OR 0x2		/* happens before AND */
#define ACT_ACCESS_AND 0x4


static void space_modify_range(
	struct space *sp,
	L4_Fpage_t range,
	int act,
	uint32_t first_pgid,
	int access)
{
	assert((access & ~L4_FullyAccessible) == 0);
	assert(check_space(NO_PTAB_TO_MAPDB, sp));
	assert(act > 0);

	uint32_t pgid_mask = ~0u;
	/* ANDing away the read bit implies removal. */
	if(CHECK_FLAG(act, ACT_ACCESS_AND)
		&& !CHECK_FLAG(access, L4_Readable))
	{
		pgid_mask = 0;
		act = ACT_SET_PGID;
	}
	if(CHECK_FLAG(act, ACT_ACCESS_OR) && (access & L4_FullyAccessible) == 0) {
		act &= ~ACT_ACCESS_OR;
	}

	/* FIXME: cur_pgid doesn't increase correctly over skipped ranges. */
	bool skipped = false;
	const uint32_t *pdir_mem = sp->pdirs->vm_addr;
	uint32_t cur_pgid = first_pgid,
		writebit = CHECK_FLAG(L4_Rights(range), L4_Writable) ? PT_RW : 0;
	L4_Word_t addr = FPAGE_LOW(range), high = FPAGE_HIGH(range);
	do {
		if(skipped) panic("FIXME needs a fixing");
		if(CHECK_FLAG(pdir_mem[addr >> 22], PDIR_PRESENT)) {
			struct page *pg = get_ptab_page(sp, pdir_mem[addr >> 22] >> 12);
			assert(pg != NULL);
			assert(pg->vm_addr != NULL);
			uint32_t *ptab_mem = pg->vm_addr;
			L4_Word_t pos = addr;
			for(int i = (addr >> 12) & 0x3ff;
				i < 0x400 && pos <= high;
				i++, pos += PAGE_SIZE, cur_pgid++)
			{
				if(CHECK_FLAG(act, ACT_SET_PGID)) {
					ptab_mem[i] = (cur_pgid << 12 | writebit
						| PT_PRESENT | PT_USER) & pgid_mask;
				}
				if(CHECK_FLAG(act, ACT_ACCESS_OR)) ptab_mem[i] |= writebit;
				if(CHECK_FLAG(act, ACT_ACCESS_AND)) {
					ptab_mem[i] &= ~PT_RW | writebit;
				}
				if(sp == current_space) x86_invalidate_page(pos);
			}
		} else {
#if 0
			printf("%s: skipped addr=%#lx, no pdir present\n",
				__func__, addr);
#endif
			skipped = true;		/* see the FIXME above */
		}

		addr = (addr + PAGE_SIZE * 1024) & ~(PAGE_SIZE * 1024 - 1);
	} while(addr <= high);
...
void space_set_range(struct space *sp, L4_Fpage_t range, uint32_t first_pgid)
{
	space_modify_range(sp, range, ACT_SET_PGID, first_pgid, L4_Rights(range));
}


void space_set_range_access(struct space *sp, L4_Fpage_t fp)
{
	space_modify_range(sp, fp, ACT_ACCESS_OR | ACT_ACCESS_AND,
		0, L4_Rights(fp));
}
